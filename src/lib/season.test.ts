...
describe("getSeasonForDate", () => {
  ...
  it("treats the first day of each season as the new season", () => {
    expect(getSeasonForDate(date("2026-03-01T00:00:00.000Z")).key).toBe(
      "AUTUMN",
    );
    expect(getSeasonForDate(date("2026-06-01T00:00:00.000Z")).key).toBe(
      "WINTER",
    );
    expect(getSeasonForDate(date("2026-09-01T00:00:00.000Z")).key).toBe(
      "SPRING",
    );
    expect(getSeasonForDate(date("2026-12-01T00:00:00.000Z")).key).toBe(
      "SUMMER",
    );
  });
});
...
describe("groupFestivalsBySeason", () => {
  ...
  it("sorts festivals within a season by start date", () => {
    ...
    expect(summer.festivals.map((f) => f.name)).toEqual([
      "Summer Fest",
      "Late Summer",
    ]);
  });
