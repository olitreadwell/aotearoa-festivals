...
export default async function FestivalsPage({
  ...
  const upcoming = (await prisma.festival.findMany({
    where: {
      ...
      OR: [
        { startDate: { gte: now } },
        { startDate: null, status: { in: [FestivalStatus.ACTIVE, FestivalStatus.TBC] } },
      ],
    },
    orderBy: [{ startDate: { sort: "asc", nulls: "last" } }, { name: "asc" }],
    include: { promoter: true },
  })) as FestivalWithPromoter[];

  const past = (await prisma.festival.findMany({
    where: {
      ...baseWhere,
      startDate: { lt: now, not: null },
    },
      ...
      <section className="mb-12">
        ...
        <p className="mb-5 text-sm text-[#555] dark:text-[#aaa]" role="status">
          {upcoming.length === 0
            ...
            : `${upcoming.length} upcoming festival${upcoming.length !== 1 ? "s" : ""}`}
        </p>
    ...
    <li>
      ...
      >
          ...
          <dl className="flex flex-col gap-1 text-sm text-[#555] dark:text-[#aaa]">
            {festival.region && (
              <div className="flex items-center gap-1.5">
                ...
                <svg aria-hidden="true" className="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9.69 18.933l.003.001C9.89 19.02 10 19 10 19s.11.02.308-.066l.002-.001.006-.003.018-.008a5.741 5.741 0 00.281-.14c.186-.096.446-.24.757-.433.62-.384 1.445-.966 2.274-1.765C15.302 14.988 17 12.493 17 9A7 7 0 103 9c0 3.492 1.698 5.988 3.355 7.584a13.731 13.731 0 002.273 1.765 11.842 11.842 0 00.976.544l.062.029.018.008.006.003zM10 11.25a2.25 2.25 0 100-4.5 2.25 2.25 0 000 4.5z" clipRule="evenodd" /></svg>
            ...
            {festival.genre && (
              <div className="flex items-center gap-1.5">
                ...
                <svg aria-hidden="true" className="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path d="M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V7.82l8-1.6v5.894A4.37 4.37 0 0015 12c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V3z" /></svg>
            ...
            {festival.dateText && (
              <div className="flex items-center gap-1.5">
                ...
                <svg aria-hidden="true" className="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" /></svg>
            ...
            {festival.promoter && (
              <div className="flex items-center gap-1.5">
                ...
                <svg aria-hidden="true" className="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="currentColor"><path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v1h8v-1zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-1a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v1h-3zM4.75 12.094A5.973 5.973 0 004 15v1H1v-1a3 3 0 013.75-2.906z" /></svg>
        ...
        <div className="flex justify-end px-5 pb-4">
          <svg aria-hidden="true" className="h-4 w-4 text-gray-400 transition-colors group-hover:text-blue-500" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" /></svg>
        </div>
      </Link>
    </li>
  );
...
function EmptyState() {
  return (
    <div className="rounded-xl border border-dashed border-gray-300 p-12 text-center text-[#555] dark:border-gray-600 dark:text-[#aaa]">
      <p className="text-sm">
        No upcoming festivals match your filters.{" "}
        <Link href="/festivals" className="text-blue-600 underline dark:text-blue-400">
          Clear filters
        </Link>
      </p>
    </div>
