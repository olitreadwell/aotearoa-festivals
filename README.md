...
The **Build your season** tool generates a non-overlapping itinerary live as you change controls: strategy (most festivals, biggest crowds by attendance, or small & intimate festivals — shown as selectable cards), region (North Island, South Island, all NZ), genre (matches the festival tag or the genres of artists in its lineup), camping availability, minimum days, and a max count. Itinerary rows show dates, region, camping, attendance, duration, and ticket price where known; picks can be added to the plan in one click. `attendance` is a rounded estimate — the bigger the crowd, the more "biggest" favours it and the less "small & intimate" does.
